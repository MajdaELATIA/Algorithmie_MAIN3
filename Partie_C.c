 ...
 *****************************************************************************************************************/
void print_graphviz(int nbrAretesParcourue, Aretes * res_sommets_decouv, char * ptfn){
  FILE * f ;
  int  i ;
  ...
  if (oriente == 0){
    ...
    for (i = 0 ; i < nbrAretesParcourue ; i ++){
      fprintf(f,"%3d -- %3d\n",res_sommets_decouv[i].depart,res_sommets_decouv[i].arrive) ;
    }
      fprintf(f,"}\n") ;
  }
  else // Graphe orienté
...
void printGraphe( Graphe G){
  ...
  for (int i = 0  ; i < G.nbrAretes ; i ++){
    ...
    j = j + 1 ;
    if (j == SIZE2) // Formatage de la sortie : limitation de la longueur de la ligne
      { printf("\n") ; j = 0 ;
      }
  }
  if (j != 0){
 ...
 *Ponts
 *****************************************************************************************************************/
int pont_graph(Graphe *G){
  int taille = G -> nbrSommets;
  for (int i = 0; i < taille; i++) {
    for (int j= 0; j < taille; j++) {
      if ((G->mat_Adj)[i+1][j+1] == 1) {
        (G->mat_Adj)[i+1][j+1] = 0;
        if ( !connexe(*G) ) {
...
int main(){
  ...
  creation_en_dot(graphe,"/home/majda/Bureau/S2_Main3/Algorithmique/TP1_2_Majda_ELATIA/Images/graph-C_ponts.txt") ;
  creation_en_dot(graphe,"/home/majda/Bureau/S2_Main3/Algorithmique/TP1_2_Majda_ELATIA/Images/graph-C_ponts.dot") ;
  printf("\n Avant suppression d'arêtes :\n");
  connexe(graphe) ;
  printf("\n Après suppression d'arêtes :\n");
  printf("\n");
  pont_graph(&graphe);
  printf("\n");
  printf("\n Affichage de la nouvelle matrice d'adjacence, une fois l'arête supprimée \n");
  printMatriceAdj(graphe) ;
  printf("\n Nouvelle liste \n");
  printlisteAdj(graphe) ;

  printf("On a fini la partie C  \n") ;
}
